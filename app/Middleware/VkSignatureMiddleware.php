...
final class VkSignatureMiddleware implements MiddlewareInterface
{
    ...
    public function process(Req $request, Handler $handler): Res
    {
        ...
        if (!isset($params['payload'], $params['sign'], $params['ts'])) {
            return Response::problem(401, 'Missing required parameters for VK signature validation');
        }

        $secret = $_ENV['VK_APP_SECRET'];
        if (empty($secret)) {
            return Response::problem(401, 'VK App Secret is not configured');
        }
        ...
        if ($payloadStr === '' || $receivedSign === '' || $ts === '') {
            return Response::problem(401, 'Empty required parameters for VK signature validation');
        }

        $appId = (int)$_ENV['VK_APP_ID'];
        if (!$appId) {
            return Response::problem(401, 'VK App ID is not configured');
        }

        parse_str($payloadStr, $payloadArray);
        $userId = $payloadArray['user_id'] ?? null;
        if ($userId === null) {
            return Response::problem(401, 'User ID not found in payload for VK signature validation');
        }
        ...
        if (!hash_equals($expectedSign, $receivedSign)) {
            return Response::problem(401, 'VK signature validation failed');
        }
